...
1. **`WB` / `PB` / `AB` / `EG` / `FC` の正式な意味** — 実測の時間差から
   `PB_ON` = 塗装ライン投入（ブース入口の約 5.2 時間前）、`PB_OK` = 塗装工程完了と**推定**しているが未確認。
   R3（date アンカー = `PB_ON`）はこの推定に依存する。config で差し替え可能にしてある。
2. **`PB_OK` がブース通過の +103h 中央値になる理由** — 修正済み車体だけを見ているための選択バイアスと推定。
   本書では `PB_OK` を使わないので影響なし。
3. **`修正工数` が全 0 の理由** — 未入力運用か、この期間だけか不明。列は保持する。
...
6. **repair の `VIN` に 17 桁フル VIN が 57 行ある**（本体設計 §13-2 と同じ論点）。本書では追加の変換を行わない。
...
1. **`修正員`（作業者氏名）をどう扱いますか。**
   既定は「convert 段階で不可逆ハッシュ化し、レイクにも成果物にも氏名を残さない」です。
   作業者要因の分析は ID ベースで可能ですが、**特定の作業者を名指しで指摘することはできなくなります**。
   氏名のまま保持（`mode: keep`）、完全除外（`mode: drop`）への変更を希望されますか。
   また、ハッシュのソルトを固定（過去データと ID が一致する）でよいですか。
2. **`PB-ON` は「塗装ラインへの車体投入時刻」で合っていますか。**
   実測でブース入口通過の約 5.2 時間前に安定していたため、repair レコードを「どの日の生産か」に
   割り当てるアンカーに使っています。より適切な列があれば教えてください。
   併せて `WB` / `PB` / `AB` / `EG` / `FC` の正式名称をご教示ください。
3. **`修正工数` が全 514 行 `0` です。** 未入力運用ですか、それとも別の単位・列に工数が入りますか。
4. **修正の分類でモデリングに使いたい粒度はどれですか。**
   既定は `大分類`（10 値）のみカウント列に展開し、`中分類`(42) / `小分類`(19) / `部品名`(39) / `部位`(135) は
   展開しません。`小分類` には「作業者要因 / 設備要因 / 特定できず」が入っており要因分析に有用そうですが、
   結果側（リーク列）なので説明変数には使えません。**目的変数として使いたい分類はどれですか。**
