...
1. **VIN サフィックス `a`/`b`/`c` の業務的意味** — 2トーン2回目 / 再塗装 / その他のいずれか不明。
   → `suffix_policy` で切替可能にし、既定 `keep`（情報を壊さない側）。`vin_base` / `vin_pass_no` を常に保持。
2. **trend 結合窓（±5分）と「入口通過時刻をアンカーにする」ことの業務的妥当性** — 未検証。
   設備の滞在時間（`各工程滞在時間` に実データあり）を使って「入口〜出口の区間平均」にするのが
   本来正しい可能性がある。→ 窓幅・モードは config で可変にしてある。
3. **1年分の配置形式** — 月次1ファイルか日次か、命名規則が変わるか不明。
   → `source_key` が `_YYYY` / `_YYYYMM` / `_YYYYMMDD` / サフィックス無しを吸収し、
   日付はファイル名ではなく**中身の日時列**から決めるので、どちらでも動く設計にしてある。
...
6. **不良の「重大度」定義** — 実データに `severity` は無い。`不良サイズ` が代替になり得るが閾値は不明。
   → 本書では閾値を設けず統計量のみ出す。
7. **ブツ検に行が無い VIN の意味**（不良ゼロ か 未検査 か）。→ 既定は 0 埋め（不良ゼロ扱い）。

---

## 14. ユーザー確認事項（要回答）

1. **VIN サフィックス `a` / `b` / `c` は何を意味しますか。** 同一車体の2回目・3回目通過ですか、別車体扱いですか。
   （既定は「別キー（別レコード）として扱う」で実装します）
2. **VIN の2形式（型式ベース `MR92S-617408` と 17桁フル VIN `JS3JB74V8V5106401`）は別の車体ですか。**
   同一車体を別表記で書いているなら、キーを統合する変換規則が必要です。
3. **ブツ検データに行が無い VIN は「不良ゼロ」ですか、「未検査」ですか。**
   （上塗ブツ検 933 VIN / 全 1218 VIN。区別できないと目的変数が作れません。既定は「不良ゼロ」として 0 埋め）
4. **trend を VIN に紐付ける基準時刻は「その設備の入口通過時刻」で妥当ですか。**
   窓は ±5分を既定にしていますが、設備滞在時間（`各工程滞在時間`）で区間平均を取るべきですか。
5. **工場共通ユーティリティ系 trend（コンベア・冷水/温水供給・作業場空調・塗料供給・塗料供給空調）は
   どの工程の通過時刻に合わせるべきですか。**（既定は「ブース入口」にフォールバックします）
6. **1年分のデータはどう配置されますか**（月次1ファイル / 日次 / 命名規則）。
7. **`閾値判定ﾌﾗｸﾞ`（値に `2` がある）と `判定結果_3Bit` の値定義**を教えてください。
8. **上塗ロボットの測定値が 0 の行は「そのロボットは塗装していない」という意味ですか。**
9. **repair（修正）データは今後提供されますか。**（現在ディレクトリが空のため optional 扱いにしています）
10. **不良の重大度に相当する指標はありますか。** `不良サイズ` に運用上の閾値があれば教えてください。
11. **trend と traceability の期間が重複するサンプルを頂けますか。** 現サンプルでは trend 結合が
